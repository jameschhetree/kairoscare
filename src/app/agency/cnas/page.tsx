        ...
        <div className="grid gap-4 md:grid-cols-2">
          {demoCNAs.map((cna) => (
            <div key={cna.id} className="card p-5">
              ...
              <p className="mt-3 text-[0.86rem] text-[color:var(--color-warm-ink)]">{cna.languages} · {cna.clientCount} client{cna.clientCount === 1 ? "" : "s"}</p>
            </div>
          ))}
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {cnas.map((cna) => (
          <div key={cna.id} className="card p-5">
            ...
            <p className="mt-3 text-[0.86rem] text-[color:var(--color-warm-ink)]">{cna.languagesSpoken.join(", ")} · {cna.clientAssignments.length} client{cna.clientAssignments.length === 1 ? "" : "s"}</p>
          </div>
        ))}
      </div>
      <ComingSoonCard phase="Phase 4" title="Caregiver management" body="Add and deactivate caregivers, see submitted updates, set agency-default languages, manage certifications." />
