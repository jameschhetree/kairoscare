    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-[color:var(--color-warm-ink)]">
          Every agency on KairosCare and their pilot status.
        </p>
      </div>
        ...
        {agencies.map((a) => (
          <div key={a.id} className="flex flex-wrap items-baseline justify-between gap-2 px-5 py-4">
            ...
            <p className="text-[0.86rem] text-[color:var(--color-warm-ink)]">
              {a._count.clients} clients · {a._count.users} users ·{" "}
              {a.subscriptionPlans[0]?.tier ?? "Pilot"}
            </p>
          </div>
        ))}
      </div>
      <ComingSoonCard
        ...
        body="Create, edit, suspend; view plan + usage; force-rotate API keys; audit drilldown per agency."
      />
    </div>
  );
}
