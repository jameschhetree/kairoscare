    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 max-w-[560px] text-[color:var(--color-warm-ink)]">
          A warm, photo-rich timeline arrives in Phase 3. Today you can see which
          loved ones are linked to your account.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {memberships.length === 0 ? (
          <div className="card p-6 text-[color:var(--color-warm-ink)]">
            No clients are linked to your account yet. Ask the agency to send an
            invite — they can do this from the agency portal.
          </div>
        ) : (
          memberships.map((m) => (
            <div key={m.id} className="card p-5">
              ...
              <p className="mt-1 text-[0.9rem] text-[color:var(--color-warm-ink)]">
                {m.client.address ?? "Address on file"}
              </p>
            </div>
          ))
        )}
      ...
      <ComingSoonCard
        ...
        body="Reverse-chronological feed of every visit moment — mood, meal, activity, photo, note. Send a thank-you reaction. Add a comment that routes to the agency, never the caregiver directly."
      />
    </div>
  );
}
