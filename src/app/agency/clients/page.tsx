    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-[color:var(--color-warm-ink)]">
          Active clients and the caregivers + families linked to each.
        </p>
      </div>
      ...
      <div className="card divide-y divide-[color:var(--color-warm-hairline)]">
        {clients.map((c) => (
          <Link
            ...
            className="flex flex-col gap-1 px-5 py-4 hover:bg-[color:var(--color-cream-100)]"
          >
            ...
            <p className="text-[0.86rem] text-[color:var(--color-warm-ink)]">
              {c.cnaAssignments.length} caregiver{c.cnaAssignments.length === 1 ? "" : "s"} ·{" "}
              {c.familyMemberships.length} family seat
              {c.familyMemberships.length === 1 ? "" : "s"}
            </p>
          </Link>
        ))}
      </div>
      ...
      <ComingSoonCard
        ...
        body="Create + edit client profiles, assign caregivers, invite family members, set agency-internal notes. Until then this list reads straight from the seeded BrightPath data."
      />
    </div>
  );
}
