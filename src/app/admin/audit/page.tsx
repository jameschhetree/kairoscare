import { getDemoRole, demoAuditRows } from "@/lib/demo-seed";
  ...
  if (isDemo) {
    return (
      <div className="space-y-8">
        <div>
          <h1 className="font-display text-[2rem] leading-[1.1] text-[color:var(--color-navy-900)]">Audit log</h1>
          <p className="mt-1 text-[color:var(--color-warm-ink)]">Last 60 PHI access and write events. Full filter UI lands in Phase 5.</p>
        </div>
        <div className="card overflow-hidden">
          <table className="w-full text-[0.86rem]">
            ...
            <tbody className="divide-y divide-[color:var(--color-warm-hairline)]">
              {demoAuditRows.map((r) => (
                <tr key={r.id}>
                  <td className="px-4 py-3 text-[color:var(--color-warm-muted)]">{r.timestamp}</td>
                  <td className="px-4 py-3">{r.actorName}</td>
                  <td className="px-4 py-3 font-mono text-[0.8rem]">{r.action}</td>
                  <td className="px-4 py-3">{r.entityType}:{r.entityId.slice(0, 8)}</td>
                  <td className="px-4 py-3 text-[color:var(--color-warm-muted)]">{r.agencyName}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="card overflow-hidden">
        <table className="w-full text-[0.86rem]">
          ...
          <tbody className="divide-y divide-[color:var(--color-warm-hairline)]">
            {rows.map((r) => (
              <tr key={r.id}>
                <td className="px-4 py-3 text-[color:var(--color-warm-muted)]">{r.timestamp.toISOString().replace("T", " ").slice(0, 19)}</td>
