    ...
    return (
      <div className="space-y-6 md:space-y-8">
        <div>
          ...
          <h1 className="mt-2 font-display text-[1.8rem] leading-[1.1] text-[color:var(--color-navy-900)] md:text-[2.6rem]">Your family</h1>
        ...
        <div className="grid gap-3 md:grid-cols-2 md:gap-4">
          {demoFamilyClients.map((fc) => (
            <div key={fc.id} className="card p-4 md:p-5">
              <p className="text-[0.74rem] uppercase tracking-[0.18em] text-[color:var(--color-warm-muted)] md:text-[0.78rem]">{fc.role}</p>
              <p className="mt-1 font-display text-[1.1rem] font-semibold text-[color:var(--color-navy-900)] md:text-[1.2rem]">{fc.clientName}</p>
              <p className="mt-1 text-[0.86rem] leading-snug text-[color:var(--color-warm-ink)] md:text-[0.9rem]">{fc.clientAddress}</p>
            </div>
          ))}
        </div>
    ...
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="grid gap-3 md:grid-cols-2 md:gap-4">
        {memberships.length === 0 ? (
          <div className="card p-5 text-[0.92rem] text-[color:var(--color-warm-ink)] md:p-6">
          ...
          return (
            <Link
              ...
              className="card block p-4 transition-colors hover:bg-[color:var(--color-cream-100)] md:p-5"
            >
              <p className="text-[0.74rem] uppercase tracking-[0.18em] text-[color:var(--color-warm-muted)] md:text-[0.78rem]">{m.role}</p>
              <p className="mt-1 font-display text-[1.1rem] font-semibold text-[color:var(--color-navy-900)] md:text-[1.2rem]">{m.client.fullName}</p>
              <p className="mt-1 text-[0.86rem] leading-snug text-[color:var(--color-warm-ink)] md:text-[0.9rem]">{m.client.address ?? "Address on file"}</p>
              <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-[color:var(--color-sky-200,#e3eaf2)] pt-3 md:gap-x-4">
                <span className="text-[0.78rem] text-[color:var(--color-warm-ink)] md:text-[0.82rem]">
                  <span className="text-[color:var(--color-warm-muted)]">Last update:</span> {formatRelativeDate(s.lastUpdate)}
                </span>
                {s.latestMood && (
                  <span className="inline-flex items-center gap-1.5 text-[0.78rem] text-[color:var(--color-warm-ink)] md:text-[0.82rem]">
                    <span
